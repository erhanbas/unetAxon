...
from unet.metrics import mask_mean_squared_error_loss, dice_coefficient_loss, \
    get_label_dice_coefficient_function, dice_coefficient, detection_metric
# K.set_image_dim_ordering('tf')
...
def generate_unet3D(input_shape, pool_size=(2, 2, 2), n_labels=1, initial_learning_rate=0.00001, deconvolution=False,
    ...
    for layer_depth in range(depth-2, -1, -1):
        ...
        current_layer = create_convolution_block(n_filters=levels[layer_depth][1]._keras_shape[-1],
                                                 ...
                                                 batch_normalization=batch_normalization)
    # for regression set target to 1
    if regress:
    ...
    model.compile(optimizer=Adam(lr=initial_learning_rate), loss=mask_mean_squared_error_loss, metrics=metrics)
